...
public class AlienManager {
	...
	private static Alien[] aliens = new Alien[40];
	private static Image[] images;
	private static Image[] imagesAlt;
	...
	public static void create(Ship target){
		...
		for(int i=0; i<aliens.length; i++){
			if(aliens[i]==null){
				...
				aliens[i] = new CloningAlien(x, y, vx, vy, images[rand], imagesAlt[rand], target);
				
				
				double rand2 = Math.random();
				if(rand2>5.0/6) aliens[i] = new RealNastyAlien(x, y, vx, vy, images[rand], imagesAlt[rand], target);
				else if(rand2>4.0/6) aliens[i] = new NastyAlien(x, y, vx, vy, images[rand], imagesAlt[rand], target);
				else if(rand2>3.0/6) aliens[i] = new MultiShootAlien(x, y, vx, vy, images[rand], imagesAlt[rand], target);
				else if(rand2>2.0/6) aliens[i] = new CloakAlien(x, y, vx, vy, images[rand], imagesAlt[rand], target);
				else if(rand2>1.0/6) aliens[i] = new CloakAlien(x, y, vx, vy, images[rand], imagesAlt[rand], target);
