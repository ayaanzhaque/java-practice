...
public class Ship implements KeyListener, MouseMotionListener {
	...
	public void keyPressed(KeyEvent e) {
		...
		if(keyCode==KeyEvent.VK_SPACE && TorpedoManager.seatAvailable()){
			...
			TorpedoManager.add( new Torpedo(x-width*2, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x-width*3, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x-width*4, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x-width*5, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x-width*6, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x-width*7, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x-width, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x-width/2, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x-width/4, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x+width/4, y-Torpedo.getHeight(), -4) );
			TorpedoManager.add( new Torpedo(x+width/2, y-Torpedo.getHeight(), -4) );
